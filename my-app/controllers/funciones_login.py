        ...
        try:
            with connectionBD() as conexion_MySQLdb:
                with conexion_MySQLdb.cursor(dictionary=True) as mycursor:
                    ...
                    id_usuario = mycursor.lastrowid  # Retorna el ID insertado
                    return id_usuario
...
def recibeInsertRegisterNino(nombre_nino, apellido_nino, edad, genero, id_terapeuta):
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as mycursor:
                sql = """
                INSERT INTO ninos (nombre_nino, apellido_nino, edad, genero, id_terapeuta)
                VALUES (%s, %s, %s, %s, %s)
                """
                valores = (nombre_nino, apellido_nino, edad, genero, id_terapeuta)
                mycursor.execute(sql, valores)
                conexion_MySQLdb.commit()
                resultado_insert = mycursor.rowcount
                return resultado_insert
    except Exception as e:
        print(f"Error en el Insert ninos: {e}")
        return False
    ...
    try:
        with connectionBD() as conexion_MySQLdb:
            with conexion_MySQLdb.cursor(dictionary=True) as cursor:
                ...
                elif not re.match(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$', pass_user):
